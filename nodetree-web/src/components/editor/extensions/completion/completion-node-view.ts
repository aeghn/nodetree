...
export type CompletionRenderFn = (text: string, element: HTMLElement) => void;

import { collapseCompletionNode } from "./commands/collapse-completion-node";
import { ICompletionPluginState } from "./completion-plugin";
...
interface ICompletionViewOptions {
  /** Dom element name to use for this NodeView */
  tagName?: string;

  /** Used to render the Tex input */
  renderer: CompletionRenderFn;

  /** Should be true if node is inline */
  className?: string;
}

export class CompletionView implements NodeView, ICursorPosObserver {
  // nodeview params
  ...
  // nodeview dom
  dom: HTMLElement;
  private _completionRenderElt: HTMLElement | undefined;
  private _completionSrcElt: HTMLElement | undefined;
  ...
  private _completionPluginKey: PluginKey<ICompletionPluginState>;
  private options: ICompletionViewOptions;
   ...
   *
   * Completion Views support the following options:
   * @option displayMode If TRUE, will render completion in display mode, otherwise in inline mode.
  ...
  constructor(
    ...
    view: EditorView,
    getPos: () => number,
    options: ICompletionViewOptions,
    completionPluginKey: PluginKey<ICompletionPluginState>
  ) {
    // store arguments
    ...
    this._getPos = getPos;
    this._completionPluginKey = completionPluginKey;
    ...
    this.dom.classList.add("completion-node");

    this._completionRenderElt = document.createElement("span");
    this._completionRenderElt.textContent = "";
    this._completionRenderElt.classList.add("completion-render");
    this.dom.appendChild(this._completionRenderElt);

    this._completionSrcElt = document.createElement("span");
    this._completionSrcElt.classList.add("completion-src");
    this.dom.appendChild(this._completionSrcElt);
  ...
  destroy() {
    ...
    if (this._completionRenderElt) {
      this._completionRenderElt.remove();
      delete this._completionRenderElt;
    }
    if (this._completionSrcElt) {
      this._completionSrcElt.remove();
      delete this._completionSrcElt;
    }
   ...
   * This helps to prevent accidental deletions of completion blocks.
   */
  ensureFocus() {
  ...
  renderCompletion() {
    ...
    if (texString.length < 1) {
      this.dom.classList.add("empty-completion");
      // clear rendered completion, since this node is in an invalid state
      while (this._completionRenderElt.firstChild) {
        this._completionRenderElt.firstChild.remove();
      }
      // do not render empty completion
      return;
    } else {
      this.dom.classList.remove("empty-completion");
    }

    // render katex, but fail gracefully
    try {
      this.options.renderer(texString, this._completionRenderElt);
      this._completionRenderElt.classList.remove("parse-error");
      this.dom.setAttribute("title", "");
    } catch (err) {
      if (err instanceof Error) {
        console.error(err);
        this._completionRenderElt.classList.add("parse-error");
  ...
  openEditor() {
    ...
    if (!this._completionSrcElt) throw new Error("_completionSrcElt does not exist!");

    // create a nested ProseMirror view
    this._innerView = new EditorView(this._completionSrcElt, {
        ...
        plugins: [
          keymap({
            ...
            Backspace: chainCommands(deleteSelection, (state) => {
              ...
              // default backspace behavior when completion node is non-empty
              if (this._node.textContent.length > 0) {
                return false;
              }
              // otherwise, we want to delete the empty completion node and focus the outer view
              this._outerView.dispatch(this._outerView.state.tr.insertText(""));
              this._outerView.focus();
              return true;
            }),
            // "Ctrl-Backspace": (state, dispatch, tr_inner) => {
            //   // delete completion node and focus the outer view
            //   this._outerView.dispatch(this._outerView.state.tr.insertText(""));
            //   this._outerView.focus();
            //   return true;
            // },
            Enter: chainCommands(
              newlineInCode,
              collapseCompletionNode(this._outerView, +1, false)
            ),
            "Ctrl-Enter": collapseCompletionNode(this._outerView, +1, false),
            ArrowLeft: collapseCompletionNode(this._outerView, -1, true),
            ArrowRight: collapseCompletionNode(this._outerView, +1, true),
            ArrowUp: collapseCompletionNode(this._outerView, -1, true),
            ArrowDown: collapseCompletionNode(this._outerView, +1, true),
          }),
        ],
      }),
    ...
    // request outer cursor position before completion node was selected
    const maybePos = this._completionPluginKey.getState(
      this._outerView.state
    )?.prevCursorPos;
    if (maybePos === null || maybePos === undefined) {
      console.error(
        "[prosemirror-completion] Error:  Unable to fetch completion plugin state from key."
      );
    }
    const prevCursorPos: number = maybePos ?? 0;

    // compute position that cursor should appear within the expanded completion node
    const innerPos =
   ...
   *
   * @param render Optionally update the rendered completion after closing. (which
