...
fn guess_numbers(input: &str) -> Vec<(BaseTime, PossibleScore)> {
    ...
    let mut parsed = vec![];
    for ele in day_formats {
        let result = NaiveDate::parse_from_str(input, ele);
        if let Ok(ts) = result {
            parsed.push((ts.into(), PossibleScore::Likely(100)))
        }
    }

    for ele in time_formats {
        let result = NaiveTime::parse_from_str(input, ele);
        if let Ok(ts) = result {
            parsed.push((ts.into(), PossibleScore::Likely(100)))
        }
    }

    for ele in date_time_formats {
        let result = NaiveDateTime::parse_from_str(input, ele);
        if let Ok(ts) = result {
            parsed.push((ts.into(), PossibleScore::Likely(100)))
        }
    }
